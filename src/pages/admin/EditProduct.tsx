...
const EditProduct = () => {
    ...
    const loadProduct = async () => {
      ...
      try {
        ...
        const propertiesArray = product.properties
          ? Object.entries(product.properties).map(([key, value]) => ({ key, value: String(value) }))
          : [{ key: "", value: "" }];

        // Convert offers to editable format
        const offersArray = product.offers && product.offers.length > 0
        ...
        setProductData({
          ...
          warrantyNumber: product.warrantyNumber || "",
          rating: product.rating?.toString() || "0",
          reviewsCount: product.reviewsCount?.toString() || "0",
          salesCount: product.salesCount?.toString() || "0",
          viewsCount: product.viewsCount?.toString() || "0",
          mainImage: product.mainImage || "",
          images: product.images && product.images.length > 0 ? product.images : [""],
          features: product.features && product.features.length > 0 ? product.features : [""],
          properties: propertiesArray.length > 0 ? propertiesArray : [{ key: "", value: "" }],
          offers: offersArray,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await productsApi.update(id, productPayload);
      ...
      toast({
        title: "خطأ في الحفظ",
        description: "فشل تحديث المنتج. الرجاء المحاولة مرة أخرى",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={() => navigate("/dashboard/products")} className="gap-2">
            <ArrowRight className="w-4 h-4" />
            رجوع
          </Button>
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">تعديل المنتج</h1>
            <p className="text-muted-foreground">قم بتحديث معلومات المنتج</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="name" name="name" value={productData.name} onChange={handleNameChange} placeholder="أدخل اسم المنتج" required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="slug">الرابط <span className="text-red-500">*</span></Label>
                  <Input id="slug" name="slug" value={productData.slug} onChange={handleChange} placeholder="رابط-المنتج" required />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="sku">رمز المنتج</Label>
                  <Input id="sku" name="sku" value={productData.sku} onChange={handleChange} placeholder="SKU-001" />
                </div>
                <div className="space-y-2">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="shortDescription">الوصف المختصر <span className="text-red-500">*</span></Label>
                <Textarea id="shortDescription" name="shortDescription" value={productData.shortDescription} onChange={handleChange} placeholder="وصف قصير للمنتج يظهر في القوائم..." rows={3} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="fullDescription">الوصف الكامل <span className="text-red-500">*</span></Label>
                <RichTextEditor
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="price">السعر (درهم) <span className="text-red-500">*</span></Label>
                  <Input id="price" name="price" type="number" step="0.01" value={productData.price} onChange={handleChange} placeholder="299.00" required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="originalPrice">السعر الأصلي (درهم)</Label>
                  <Input id="originalPrice" name="originalPrice" type="number" step="0.01" value={productData.originalPrice} onChange={handleChange} placeholder="399.00" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="stockQuantity">كمية المخزون <span className="text-red-500">*</span></Label>
                  <Input id="stockQuantity" name="stockQuantity" type="number" value={productData.stockQuantity} onChange={handleChange} placeholder="100" required />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="badge">الشارة</Label>
                  <Select value={productData.badge} onValueChange={(value) => handleSelectChange("badge", value)}>
                    <SelectTrigger><SelectValue placeholder="بدون شارة" /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="warrantyNumber" name="warrantyNumber" value={productData.warrantyNumber} onChange={handleChange} placeholder="30" />
                </div>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="rating">التقييم</Label>
                  <Input id="rating" name="rating" type="number" step="0.1" min="0" max="5" value={productData.rating} onChange={handleChange} placeholder="4.5" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="reviewsCount">عدد المراجعات</Label>
                  <Input id="reviewsCount" name="reviewsCount" type="number" value={productData.reviewsCount} onChange={handleChange} placeholder="0" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="salesCount">عدد المبيعات</Label>
                  <Input id="salesCount" name="salesCount" type="number" value={productData.salesCount} onChange={handleChange} placeholder="0" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="viewsCount">عدد المشاهدين</Label>
                  <Input id="viewsCount" name="viewsCount" type="number" value={productData.viewsCount} onChange={handleChange} placeholder="0" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {productData.features.map((feature, index) => (
                <div key={index} className="flex gap-2">
                  <Input value={feature} onChange={(e) => handleFeatureChange(index, e.target.value)} placeholder={`الميزة ${index + 1} (مثال: "تصميم مقاوم للماء")`} />
                  <Button type="button" variant="outline" size="icon" onClick={() => handleRemoveFeature(index)} disabled={productData.features.length === 1}>
                    <X className="w-4 h-4" />
                  </Button>
                </div>
              ))}
              <Button type="button" variant="outline" onClick={handleAddFeature} className="w-full gap-2">
                <Plus className="w-4 h-4" />
                إضافة ميزة
              </Button>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              {productData.properties.map((property, index) => (
                <div key={index} className="flex gap-2">
                  <Input value={property.key} onChange={(e) => handlePropertyChange(index, "key", e.target.value)} placeholder="الخاصية (مثال: الحجم)" className="flex-1" />
                  <Input value={property.value} onChange={(e) => handlePropertyChange(index, "value", e.target.value)} placeholder="القيمة (مثال: 50 مل)" className="flex-1" />
                  <Button type="button" variant="outline" size="icon" onClick={() => handleRemoveProperty(index)} disabled={productData.properties.length === 1}>
                    <X className="w-4 h-4" />
                  </Button>
                </div>
              ))}
              <Button type="button" variant="outline" onClick={handleAddProperty} className="w-full gap-2">
                <Plus className="w-4 h-4" />
                إضافة خاصية
              </Button>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex gap-3">
              <Button type="button" variant="outline" onClick={() => navigate("/dashboard/products")} className="flex-1">إلغاء</Button>
